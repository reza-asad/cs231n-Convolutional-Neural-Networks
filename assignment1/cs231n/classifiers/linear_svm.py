...
def svm_loss_naive(W, X, y, reg):
  ...
  for i in xrange(num_train):
    ...
    for j in xrange(num_classes):
      ...
      if margin > 0:
        loss += margin
        positive_margin_counts += 1
        dW[:,j] += X[i]
    dW[:,y[i]] += (-1)*positive_margin_counts * X[i]
...
def svm_loss_vectorized(W, X, y, reg):
  ...
  is_margin_positive[np.arange(len(is_margin_positive)), y] = -positive_margin_counts
